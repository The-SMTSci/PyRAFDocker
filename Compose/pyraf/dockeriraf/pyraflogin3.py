...
#############################################################################
def newcube(filename,filelist,header=None,makeall=False,snr=[],slices=()):
    ...
    slices = tuple of np.s_[start,stop,step] instances. in NAXIS2 then NAXIS1 order]
    """
    msg = ""
    ...
    for fname in filelist:
        with fits.open(fname) as f:
            ...
            shapes.setdefault(d.shape.__str__(),[]).append(d)
    ...
    nf.header = header
    nf.writeto(filename,output_verify='fix',overwrite=True)
    print(f"""# {filename} written. !ds9 {filename} &""")
    if(makeall):
...
def heditdefaults():
    """Set the hedit/nhedit defaults to add/update only."""
    ...
    iraf.nhedit.add      = iraf.yes                           # nail in nhedit options
    iraf.nhedit.addonly  = iraf.no                            # as the same
